...
# 색종이가 놓인 상태는 가장 왼쪽 아래 칸의 번호와 너비, 높이를 나타내는 네 정수로 표현한다. 예를 들어, 위 그림에서 회색으로 표시된 색종이는 (1,4)가 가장 왼쪽 아래에 있고 너비 3, 높이 2이므로 1 4 3 2로 표현한다. 
...
arr = [[None]*M for _ in range(M)] # 가로 세로 1001칸인 배열 만들기
...
count = [0]* N # 색종이 개수 만큼 카운트 자리 만들어주기
