# 문제
# 정보 초등학교에서는 단체로 2박 3일 수학여행을 가기로 했다. 여러 학년이 같은 장소로 수학여행을 가려고 하는데 1학년부터 6학년까지 학생들이 묵을 방을 배정해야 한다. 남학생은 남학생끼리, 여학생은 여학생끼리 방을 배정해야 한다. 
...
# 6학년	수지	동건
# 입력
# 표준 입력으로 다음 정보가 주어진다. 첫 번째 줄에는 수학여행에 참가하는 학생 수를 나타내는 정수 N(1 ≤ N ≤ 1,000)과 한 방에 배정할 수 있는 최대 인원 수 K(1 < K ≤ 1,000)가 공백으로 분리되어 주어진다. 
...
# print(arr) # [[1, 2, 1, 0, 1, 1], [2, 1, 3, 1, 2, 1]]
...
for row in range(2):
    for col in range(6):
        if arr[row][col] !=0:
            if arr[row][col] % Max:
                count += arr[row][col] // Max + 1
            else:
                count += arr[row][col] // Max
